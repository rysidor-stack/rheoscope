...
1. The overnight scheduled run did not finish last night. It stopped partway through, which was surprising because the previous four nights all completed. Until someone reruns it tonight's metrics history will show a one-day gap. Nothing else in the project is affected by the gap. Restarting the scheduled task fixes this — the system can retry it automatically next session with a yes.
