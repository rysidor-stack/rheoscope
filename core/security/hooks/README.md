...
# THE WHOLE BOARD IN ONE COMMAND (v3.0.43): 84 assertions -- 43 embedded cases
# (DENY + its negatives, named-tool ASK, inline-with-token ASK, inline-without-token
# silent) plus every committed fixture against its pinned expectation. The driver
# lives in the hook itself, like scan-staged-secrets.sh --self-test.
./hooks/block-dangerous-bash.sh --self-test
# expect: "block-dangerous-bash self-test: 84 passed, 0 failed", exit 0
