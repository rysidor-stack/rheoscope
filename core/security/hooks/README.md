...
|---|---|---|
| **Mediated lanes refuse** (both hooks) | Edit/Write on any class path → DENY. Bash/PowerShell: any *write-shaped* command whose write **TARGET** is a class path → DENY (v3.0.51, backlog v3.0-144 / fleet inbox #4 — the v3.0.46 rule matched the token anywhere in the text, so prose merely CITING a surface path could not be written on the Bash lane; target zones: every `>`/`>>` redirect target except the harmless `2>/dev/null`/`2>&1`/`>NUL`, the operands of tee/cp/mv/rm/install/truncate/ln/dd/rsync and of the PowerShell writers Set-Content/Add-Content/Out-File/Copy-Item/Move-Item/Remove-Item/New-Item/Clear-Content/Rename-Item + aliases, and `sed -i`/`perl -i` operands). Interpreter one-liners keep whole-text matching — `py/python -c` / `node -e` / `[IO.File]::Write*` that so much as mentions the path still denies (the target hides inside code no regex parses; accepted FP). Reads pass (cat/grep/ls/diff/git show/log/diff/`sed -n`, running the sensor as a script); so does prose naming a surface while writing elsewhere (a raw/ note citing `deploy/trust.py` is knowledge intake, not a perimeter write). DENY not ASK: perimeter edits are operator-out-of-session; there is no in-session "yes". | Stop a composed spelling (`$(echo …)`, concatenated fragments, a script the agent writes and then runs). A regex is a tripwire, never a bar. |
...
| **The boundary under `visible`: one promote action outside the session, and a pending list nothing can hide** (v3.0.50, ADR #11 Release 2 / condition 4 as amended; backlog v3.0-139) | A session PREPARES a retirement (`deploy/retire.py --propose`: an immutable commit `C` on `refs/retire/<seq>` built with git plumbing — the working tree and the branch are never touched, so a prepared retirement is complete-or-absent) and stops. The operator runs `py deploy/promote.py <proposal-digest>` from their own terminal: it refuses under the session environment markers, re-derives `C` from objects, writes the PROMOTION RECORD (an annotated tag `retire/<seq>` carrying the exact `proposal_digest` and `mode: visible`), and fast-forwards the branch atomically; `trust.py check_publishable` accepts that record in place of the signed tag under `visible` only (a wrong, stale or consumed digest refuses). In-session, the Bash lane DENIES running `promote.py` and `git tag … retire/<n>` (reads, `retire.py`, `pending.py`, `git tag -l/-v` pass). Independently, `deploy/pending.py` RECONSTRUCTS every retirement and every trust-surface commit on the branch's first-parent chain from git objects (commit, parent, author, proposal digest, diff identity, cold-object identity) into a pending list that stays outstanding until an ATTENDED sweep renders it (reading the sweep is the acknowledgement; an unattended sweep renders but cannot ack); a missed observation window (`project.yaml: observation_window_days`, default 7) or a failed sweep cycle becomes an alarm row written by the nightly standing-loop tick. `/doctor` check 16 (f)/(g)/(h) and `/sweep` step 17 carry it. | Prove a human ran `promote.py` (the stated threat is a misled, wrong or unattended session, not an adaptive adversary: an unmediated spelling that writes the same tag object is indistinguishable, and LANDS IN THE PENDING LIST — refuse or durable notice, never silence). Stop a same-user agent from appending a forged ack in an attended session — the ack names its run and briefing, and an ack for an item not in history or dated before it is a finding. |

Battery: `bash block-env-writes.sh --self-test` (56: every surface deny + allow-sibling, path
spellings, `.env` rules, class-file == floor, all fixtures), `bash block-dangerous-bash.sh
--self-test` (232: the v3.0.50 board + the v3.0-144 target-operand pins in both directions —
prose mentions pass, targets still deny, the composed-spelling standing boundary and the
sed-script accepted FP pinned), `python deploy/trust.py --self-test` (114 —
see its closing NOTE on what a battery can and cannot exercise without a FIDO token),
`deploy/retire.py --self-test` (84), `deploy/pending.py --self-test` (37),
`deploy/promote.py --self-test` (13). Acceptance artifacts: `harness-v3.0/stranger-tests/
v3.0.46-adversarial-e2e-*.md` (the `required` cases, re-run every release) and
`v3.0.50-g2-e2e-*.md` with its clean out-of-tree oracle `g2-oracle.py` (dev repo).

## Hooks in this perimeter

| Hook | Matchers required | Behavior | Risk |
|------|--------------------|--------|------|
| block-dangerous-bash.sh | `Bash`, `PowerShell` (both) | **Two tiers (v3.0.33, backlog v3.0-95 — the v3.0.19 deny-only-the-unrecoverable doctrine applied to the hook layer).** **DENY (exit 2):** destructive commands — `rm -rf /`, `git reset --hard`, the PowerShell analog `Remove-Item -Recurse -Force <bare drive/POSIX root>` (matched on the quote-normalized command since v3.0.46), the agent `--no-verify` bar, and — since v3.0.46 (v3.0-120) — **any write-shaped command whose write TARGET is a trust-surface path** (narrowed from whole-text matching in v3.0.51, v3.0-144; interpreter one-liners keep whole-text — see the class section above). No allowlist, no ask — there is no legitimate unattended "yes" to these. **ASK (exit 0 + PreToolUse `"ask"` JSON) — UNATTENDED runs only since v3.0.47 (backlog v3.0-134; the 2026-08-22 five-pass run): in an ATTENDED session every egress-shaped call is ALLOWED and appended to `.claude/egress-log.jsonl` (per-machine, gitignored, path fixed relative to the hook), which `/sweep` step 18 renders as one table per host; unattended := the scheduled wrapper sets `RHEOSCOPE_UNATTENDED=1` or the payload's `permission_mode` is `dontAsk`, and there the ask fails closed as before. Every ask and every deny is logged too (v3.0-136 telemetry). Reason, stated plainly: the threat on a one-operator harness is a misled or wrong session, not an adaptive adversary; the per-call prompt never stopped a composed spelling and its tax trained a reflexive Allow. The ask tier's original contract, now unattended-only:** network egress — `curl`/`wget`/`nc`/`netcat`, PowerShell `Invoke-WebRequest`/`Invoke-RestMethod`/`Start-BitsTransfer`/`irm`/`iwr`, and interpreter one-liners (`py`/`python`/`python3 -c`, `node -e`) **when they carry a network-shaped token** (v3.0.43, backlog v3.0-124 — operator-ratified): `urllib`/`requests`/`socket`/`http(s)`/`fetch`/`axios`/`require(net|http|tls|dgram)`/etc., matched on the quote-normalized command. A provably-local one-liner (`py -c "import json; json.load(open('x.json'))"`) passes silently — asking on every inline call taxed the operator with false positives, and prompt fatigue trains a reflexive Allow, which erodes the tier it was meant to strengthen. **Stated honestly: this NARROWS ask coverage** — pre-v3.0.43 every inline `-c`/`-e` prompted; a token-absent egress spelling (exotic stdlib API, composed import) now passes where it previously asked. That is a deliberate operator-ratified loosening (v3.0-124), same trade class as v3.0.33's deny→ask; the token list is a floor of common spellings, not an enumeration. Sessions should still prefer the Read tool / `jq` over interpreter one-liners for local file reads. The operator reviews the exact command and approves or declines that one call; **an unanswered ask fails closed**, so unattended runs stay fully perimetered. Standing allowances live in `egress-allowlist.txt` beside the script (one extended regex per line, consulted by the ASK tier only, matched-command allowed silently) — **operator-edited only**, same doctrine as `credential-bindings.yaml`; the path is fixed relative to the script on purpose (an env-settable path would let a session point the hook at its own permissive file). A command matching both tiers is DENIED (deny checked first). All matching case-insensitive. **Battery:** `bash block-dangerous-bash.sh --self-test` (v3.0.43; 84 assertions, grown to 169 in v3.0.46) — embedded driver + all committed fixtures pinned, intercepted before the stdin read so the PreToolUse path is byte-unaffected. | Egress can exfiltrate — including the cmdlet and inline-interpreter bypasses of the named-tool curl/wget match — but it is reviewable-before-run, so it asks instead of dead-ending authorized work into "disable the hook and restart" (the v3.0-95 incident). Destructive commands can wipe state and are unrecoverable, so they stay denied. Inline `-c`/`-e` only (scripts like `python build.py` are allowed); the root-targeting rule matches only a bare root token (`C:\`, `/`, ...) with nothing after it — inert-unless-real-risk. |
...
bash hooks/scan-staged-secrets.sh --self-test   # expect PASS (51/51)
```
