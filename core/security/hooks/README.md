...
--self-test` (222: the prior 199 + the in-session promotion DENY with its read/propose/list
negatives, write-shaped × the three new surfaces), `python deploy/trust.py --self-test` (114 —
see its closing NOTE on what a battery can and cannot exercise without a FIDO token),
`deploy/retire.py --self-test` (63), `deploy/pending.py --self-test` (26),
